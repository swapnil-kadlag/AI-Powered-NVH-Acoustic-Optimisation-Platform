...
def llm_answer(question, hits):
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "You are an expert NVH (Noise, Vibration & Harshness) engineer "
                    "specialising in microwave oven acoustics. Answer questions based "
                    "on the provided context from IEC standards, SPR methodology and "
                    "NVH mitigation literature. Be concise, technically precise and practical."
                )},
...
# ───────────────────────────────────────────────────────────────
with tabs[6]:
    st.markdown('<p class="section">NVH Knowledge Assistant (RAG + GPT-4o-mini)</p>',
                unsafe_allow_html=True)
    has_llm = _get_openai_key() is not None
    if has_llm:
        st.caption("Powered by BM25 retrieval over IEC standards, SPR methodology "
                   "& mitigation library — answers generated by GPT-4o-mini.")
    else:
        st.caption("Powered by BM25 retrieval (offline). Add OPENAI_API_KEY to Streamlit "
                   "secrets to enable GPT-4o-mini answers.")
            ...
            if hits:
                ...
                if has_llm:
                    with st.spinner("Generating answer with GPT-4o-mini …"):
                        answer = llm_answer(user_in, hits)

                if answer:
                    st.markdown(answer)
                    with st.expander("📚 Source chunks used for this answer"):
                        for h in hits:
                            st.markdown(f"**{h['source']}** | relevance score: {h['score']:.3f}")
                            st.text(h["chunk"])
                            st.divider()
                    st.session_state.messages.append({"role": "assistant", "content": answer})
                else:
                    response = "**Retrieved from NVH Knowledge Base:**\n\n"
